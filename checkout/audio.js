...
(()=>{
 ...
 function ping(){
  ...
  try{const t=context.currentTime;
   [880,1320].forEach((frequency,i)=>{const o=context.createOscillator(),g=context.createGain(),start=t+i*.085;
    o.type='sine';o.frequency.value=frequency;g.gain.setValueAtTime(0,start);g.gain.linearRampToValueAtTime(.12,start+.008);g.gain.exponentialRampToValueAtTime(.001,start+.24);
    o.connect(g).connect(context.destination);o.start(start);o.stop(start+.25);o.onended=()=>{o.disconnect();g.disconnect();};
   });
  }catch{}
 }
 function tick(){
  if(muted||!context||context.state!=='running'||document.hidden)return;
  try{const t=context.currentTime,o=context.createOscillator(),g=context.createGain();
   o.type='square';o.frequency.setValueAtTime(1250,t);o.frequency.exponentialRampToValueAtTime(650,t+.045);
   g.gain.setValueAtTime(.055,t);g.gain.exponentialRampToValueAtTime(.001,t+.055);
   o.connect(g).connect(context.destination);o.start(t);o.stop(t+.06);o.onended=()=>{o.disconnect();g.disconnect();};
  }catch{}
 }
 window.SPL_DART_AUDIO={play,complete,tick,ping,bust:()=>complete('bust'),stopCompletion};render();
