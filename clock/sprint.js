...
(() => {
 ...
 function release(source){
  ...
  pending={kind:'throw',name:'clock_throw',power,args:{p_game_id:game.id,p_request_id:crypto.randomUUID(),p_aim_x:aim.x+wobble.x,p_aim_y:aim.y+wobble.y,p_power:power,p_dart_no:game.total_darts}};runPending();
 }
 ...
 guestButton.onclick=()=>{if(user)return;authCheck++;epoch++;stopAutoCollect();cancel();pending=null;busy=false;game=null;hits=[];shots=[];guest=window.SPL_GUEST.create('clock');player={name:'Practice',avatar_url:null};identity();update();start();};
 ...
 if(guest){$('authMessage').textContent='Practice · local play';$('restart').textContent='NEW PRACTICE GAME';$('state').textContent='PRACTICE';if(game?.status==='completed'){$('finishRank').textContent='Practice result';if('clock'==='sprint')$('checkout').textContent='GAME SHOT';}}
 if(game?.status==='completed'){const rows=[['Time',time(game.elapsed_ms)],['Darts',game.total_darts],['Targets','1–20 + outer & inner bull'],['Hit rate',(100*game.matched_darts/game.total_darts).toFixed(1)+'%']];resultCard.set({id:game.id,mode:'Around the Clock',playerName:guest?'':player?.name,headline:time(game.elapsed_ms),rows});}else resultCard.set(null);
 }
 let boardLoad=0;
