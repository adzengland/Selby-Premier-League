...
async function send(id,p){try{const data=await rpc(p.kind==='start'?'fixture_score_start':'fixture_score_submit',p.kind==='start'?{p_fixture_id:id,p_first:p.first,p_request_id:p.request}:{p_fixture_id:id,p_revision:p.revision,p_request_id:p.request,p_darts:p.darts,p_start_request:p.startRequest});const result=unpack(id,data,p.kind==='start');clear(id);return result;}catch(e){if(['P0001','40001','42501','22023','22P02'].includes(e.code))clear(id);throw e;}}
window.SPLFixtureAPI={pending,
 ...
 async submit(id,revision,darts){if(pending(id))throw Error('Resolve the pending submission before entering another visit.');const m=cache.get(id);if(!m)throw Error('Reload the match before submitting.');const p={kind:'visit',revision,startRequest:m.start_request,darts:darts.map(({n,m})=>({n,m})),request:crypto.randomUUID()};sessionStorage.setItem(key(id),JSON.stringify(p));return send(id,p);},
