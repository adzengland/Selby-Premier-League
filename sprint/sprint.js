/* SPL 501 Sprint v1. Supabase RPCs own identity, shots, visits, timing and results. */
(() => {
 'use strict';
 ...
 function update(){
  ...
  const n=game?.remaining??501,darts=game?.total_darts??0;
  $('score').textContent=n;
  ...
  document.querySelectorAll('[data-average]').forEach(e=>e.textContent=darts?((501-n)/darts*3).toFixed(2):'0.00');
  ...
  $('checkout').textContent=game?.status==='completed'?'GAME SHOT · SAVED':n<=40&&n%2===0?`FINISH ON D${n/2}`:n===50?'FINISH ON BULL':'DOUBLE OUT REQUIRED';
  ...
  if(game?.status==='completed'){
   $('finishMetrics').replaceChildren(...[['DARTS',game.total_darts],['TIME',time(game.elapsed_ms)],['AVERAGE',(1503/game.total_darts).toFixed(2)]].map(([label,value])=>{const item=document.createElement('div'),number=document.createElement('strong'),caption=document.createElement('small');number.textContent=value;caption.textContent=label;item.append(number,caption);return item;}));
  }
 ...
 async function checkAuth(){
  ...
  if(!next){
   ...
   $('loginLinks').hidden=false;$('authBanner').hidden=false;notice('Sign in to start a ranked leg.');update();return;
  }
  ...
  const {data:profile,error:profileError}=await client.rpc('sprint_player');
  if(check!==authCheck)return;
  player=profileError?null:profile;identity();
  $('authMessage').textContent=profileError?'This account cannot play yet: '+friendly(profileError):`Signed in as ${player.name}. Ranked legs are saved automatically.`;
  $('loginLinks').hidden=!profileError;$('authBanner').hidden=false;if(player)loadBoard();
  notice(profileError?'Ask the league organiser to check the player link and Sprint migration.':'Start a ranked leg when you are ready. The clock starts on your first throw.');update();
 }
 ...
 async function runPending(){
  ...
  try{
   ...
   if(op.kind==='start'){
    hits=[];shots=[];aim={x:621,y:360};power=0;notice(data.status==='active'?'Leg started. Hold THROW or SPACE, then release.':'This leg was replaced in another tab. Start a new ranked leg.');
    ...
    setTimeout(()=>{
     ...
     if(!h.bust&&data.visit_score===180&&data.darts_in_visit===3)window.SPL_EFFECTS?.maximum();
    ...
    if(data.status==='completed'){
     ...
     notice(`GAME SHOT · ${data.total_darts} darts · ${time(data.elapsed_ms)} · Saved to the leaderboard.`);loadBoard();
    }else notice(h.bust?`BUST · Back to ${data.remaining}. Next visit in 1 second.`:`${hitLabel(h)} · ${h.score} points · ${h.label==='BOUNCER'?'WIRE HIT':op.power<.20?'TOO LITTLE POWER · FELL SHORT':op.power<.46?'LOW POWER · DROPPED LOW':op.power>.54?'HIGH POWER · CLIMBED HIGH':'SWEET SPOT'}${data.pending_collect?' · Next visit in 1 second.':''}`);
   }
  }catch(error){
   if(version===epoch){
 ...
 function start(){
  ...
  window.SPL_DART_AUDIO?.stopCompletion();stopAutoCollect();cancel();pending={kind:'start',name:'sprint_start',args:{p_request_id:crypto.randomUUID()}};runPending();
 }
 function collect(){
  if(!game?.pending_collect||game.status!=='active'||busy||pending)return;
  stopAutoCollect();
  pending={kind:'collect',name:'sprint_collect',args:{p_game_id:game.id,p_visit_no:game.visit_no}};runPending();
 }
 ...
 guestButton.onclick=()=>{if(user)return;authCheck++;epoch++;stopAutoCollect();cancel();pending=null;busy=false;game=null;hits=[];shots=[];guest=window.SPL_GUEST.create('sprint');player={name:'Practice',avatar_url:null};identity();update();start();};
 ...
 if(guest){$('authMessage').textContent='Practice · local play';$('restart').textContent='NEW PRACTICE GAME';$('state').textContent='PRACTICE';if(game?.status==='completed'){$('finishRank').textContent='Practice result';if('sprint'==='sprint')$('checkout').textContent='GAME SHOT';}}
 if(game?.status==='completed'){const rows=[['Darts',game.total_darts],['Time',time(game.elapsed_ms)],['3-dart average',(1503/game.total_darts).toFixed(2)],['Highest visit',game.highest_visit]];resultCard.set({id:game.id,mode:'501 Sprint',playerName:guest?'':player?.name,headline:game.total_darts+' darts',rows});}else resultCard.set(null);
 }
 let boardLoad=0;
 async function loadBoard(){
  ...
  try{
   const {data,error}=await client.rpc('sprint_leaderboard');if(version!==boardLoad)return;if(error)throw error;
   if(game?.status==='completed'){const standing=(data||[]).find(r=>r.player_name===player?.name);$('finishRank').textContent=standing?`Overall standing: #${standing.rank} · based on your best leg`:'Standing unavailable — refresh the leaderboard to retry.';}
   const rows=(data||[]).map(r=>{
    const tr=document.createElement('tr');
    [r.rank,r.player_name,r.darts,time(r.elapsed_ms),r.three_dart_average,r.highest_visit].forEach(value=>{const td=document.createElement('td');td.textContent=value;tr.append(td);});
